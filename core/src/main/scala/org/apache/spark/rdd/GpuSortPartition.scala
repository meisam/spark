...
class GpuSortPartition[T <: Product : TypeTag, U <: Product : TypeTag]
...
*/
    ...
    if (sortColType.tpe =:= typeOf[Int]) {
      ...
      clSetKernelArg(setKeyKernel, 1, baseSize(sortColType), Pointer.to(Array(alignedPow2Size)))
      clEnqueueNDRangeKernel(context.queue, setKeyKernel, 1, null, global_work_size,
        local_work_size, 0, null, null)

      debugGpuBuffer(gpuKey, alignedPow2Size, f"gpuKey after ${setKeyKernelName}")(sortColType)
      ...
      clSetKernelArg(sortKeyKernel, 5, SHARED_SIZE_LIMIT * Sizeof.cl_int, null)
      clSetKernelArg(sortKeyKernel, 6, SHARED_SIZE_LIMIT * Sizeof.cl_int, null)

      debugGpuBuffer(gpuSortedKey, alignedPow2Size, "gpu sorted keys after sort_key_")(sortColType)
      debugGpuBuffer(gpuPos, alignedPow2Size, "gpuPos after sort_key_")(sortColType)
